...
const TILE = `group glass rounded-2xl border border-accent/10 p-5
              ...
              hover:shadow-[0_16px_50px_rgba(212,162,78,0.22)]
...
export default function ContactPageContent({ cvHref = '/cv/CV.pdf' }: { cvHref?: string }) {
  ...
  const info = t.raw('info') as {
    location: string; email: string; emailWork: string; phone: string; phoneHref: string;
  };
    ...
    <section className="max-w-7xl mx-auto px-6 py-12 relative">
      ...
      <div className="absolute inset-0 opacity-10 pointer-events-none">
        <svg viewBox="0 0 1000 500" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M220 110 C260 120 300 135 340 145 C380 155 420 165 460 170" fill="none" stroke="rgba(212,162,78,0.14)" strokeWidth="1" />
        </svg>
      </div>
            ...
            <motion.div whileHover={HOVER} className={TILE}>
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <a href={`mailto:${info.email}`} className="text-sm font-semibold text-text-primary hover:text-accent transition-colors break-all">{info.email}</a>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div whileHover={HOVER} className={TILE}>
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <a href={`mailto:${info.emailWork}`} className="text-sm font-semibold text-text-primary hover:text-accent transition-colors break-all">{info.emailWork}</a>
                </div>
              </div>
            </motion.div>
