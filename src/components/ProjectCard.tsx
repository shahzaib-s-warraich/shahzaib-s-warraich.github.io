...
export default function ProjectCard({
  ...
  // "Public" reflects an openly available source (repo or paper) — a product
  // demo link alone (e.g. a proprietary company product page) doesn't make
  // the underlying work public.
  const isPublic = !!(github || paper);
  ...
  return (
    <motion.article
      ...
      className={`group glass rounded-xl overflow-hidden border transition-colors duration-300 flex flex-col cursor-pointer
        ${featured
          ? 'border-accent/25 hover:border-accent/55 hover:shadow-[0_12px_40px_rgba(47,102,144,0.25)] hover:bg-white/[0.02]'
          : 'border-border hover:border-accent/35 hover:shadow-[0_12px_40px_rgba(47,102,144,0.20)] hover:bg-white/[0.03]'
        }`}
    >
      {/* Image */}
          ...
          <div className="flex flex-wrap items-center gap-2">
            {github && (
              <a
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium bg-accent text-bg-primary hover:bg-accent-muted transition-colors duration-200 shadow-[0_8px_20px_rgba(47,102,144,0.25)]"
              >
